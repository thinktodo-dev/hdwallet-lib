...
import 'package:hdwallet_lib/bs58check.dart' as bs58check;
import './utils/wif.dart' as wif;
import 'dart:convert';



final _finalBitcoin =  bitcoin;
const _constHighestBit = 0x80000000;
const constUnit31Max = 2147483647; // 2^31 - 1
const _constUnit32Max = 4294967295; // 2^32 - 1
...
class BIP32 {
  ...
  String toPrivateBase58() {
    final version =  network.bip32.private ;
    Uint8List buffer =  Uint8List(78);
    ByteData bytes = buffer.buffer.asByteData();
    bytes.setUint32(0, version);
    bytes.setUint8(4, depth);
  ...
  BIP32 derive(int index) {
    if (index > _constUnit32Max || index < 0) throw  ArgumentError("Expected UInt32");
    final isHardened = index >= _constHighestBit;
  ...
  BIP32 deriveHardened(int index) {
    if (index > constUnit31Max || index < 0) throw  ArgumentError("Expected UInt31");
    return derive(index + _constHighestBit);
  }
